    ...
    fun `無効なメール形式で問い合わせエンドポイントにアクセスすると400を返す`() = testApplication {
        ...
        jsonClient
                .post("/api/contact") {
                    contentType(ContentType.Application.Json)
                    setBody(
                            """{"email":"invalid-email","subject":"テスト件名","message":"これはテストメッセージです。","turnstileToken":"test-token"}"""
                    )
                }
                .apply {
                    assertEquals(HttpStatusCode.BadRequest, status)
    ...
    fun `件名が短すぎる場合は問い合わせエンドポイントが400を返す`() = testApplication {
        ...
        jsonClient
                .post("/api/contact") {
                    contentType(ContentType.Application.Json)
                    setBody(
                            """{"email":"test@example.com","subject":"ab","message":"これはテストメッセージです。","turnstileToken":"test-token"}"""
                    )
                }
                .apply {
                    assertEquals(HttpStatusCode.BadRequest, status)
    ...
    fun `メッセージが短すぎる場合は問い合わせエンドポイントが400を返す`() = testApplication {
        ...
        jsonClient
                .post("/api/contact") {
                    contentType(ContentType.Application.Json)
                    setBody(
                            """{"email":"test@example.com","subject":"テスト件名","message":"短い","turnstileToken":"test-token"}"""
                    )
                }
                .apply {
                    assertEquals(HttpStatusCode.BadRequest, status)
